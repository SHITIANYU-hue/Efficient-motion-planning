function [flag_output, output_args, IDM_output] = run(ego, obs, lane_width )
    output_args = [];
    IDM_output = [];
    flag_output = true;
    close all;
    ...
    x_obstacle = obs(1);
    x_obstacle_ = repmat(obs(1), N+1, 1);
    v_obstacle = obs(2)/3.6;
    y_obstacle = zeros(N + 1, 1);
    theta_obstacle = ones(51,1) * 0/180*pi;

    x1_obstacle = obs(4);
    x1_obstacle_ = repmat(obs(4), N+1, 1);
    v1_obstacle = obs(5)/3.6;
...
%% while loop
    ...
    %        A(i*nx+1:(i+1)*nx,(N+1)*nx+i*nu+1:(N+1)*nx+(i+1)*nu) = BB1;
        ...
        for i = 1:1:N
            ...
            v_obstacle = a_obstacle * dt + v_obstacle;
            x_obstacle = v_obstacle * dt + x_obstacle;
            % IDM for x1_obstace
            % the following car is following the leading car
            v0 = 30/3.6;
            a = a_max;  % maximum acceleration
            a1_obstacle = a * (1 - (v1_obstacle / v0)^4);
            v1_obstacle = a1_obstacle * dt + v1_obstacle;
            x1_obstacle = v1_obstacle * dt + x1_obstacle;
            % IDM for x2_obstacle
            ...
            x_obstacle_(i) = x_obstacle;
            v_obstacle_(i) = v_obstacle;
            a_obstacle_(i) = a_obstacle;
            x1_obstacle_(i) = x1_obstacle;
            v1_obstacle_(i) = v1_obstacle;
            a1_obstacle_(i) = a1_obstacle;
            x2_obstacle_(i) = x2_obstacle;
            v2_obstacle_(i) = v2_obstacle;
            a2_obstacle_(i) = a2_obstacle;
            if is_exist_1 == true
                F = F + implies(y((i-1)*nx+2) - 1.7/2 <= lane_width/2, y((i-1)*nx+1) + 3 * y((i-1)*nx+3) <= x_obstacle);
            end
            if is_exist_2 == true
                F = F + implies(-y((i-1)*nx+2) - 1.7/2 <= -lane_width/2, y((i-1)*nx+1) + 3 * y((i-1)*nx + 3) <= x1_obstacle);
            end
            if is_exist_3 == true
...
%         hold on;
%         if is_exist_1 == true
%             obstacle_x_temp = x_obstacle_(t_num) - 4.4/2;
...
%         end
%         if is_exist_2 == true
%             obstacle_x_temp = x1_obstacle_(t_num) - 4.4/2;
...
%         plot(v_pos, '-.');
    ...
    IDM_output = [x_obstacle_, v_obstacle_', a_obstacle_',...
                  x1_obstacle_, v1_obstacle_', a1_obstacle_',...
                  x2_obstacle_, v2_obstacle_', a2_obstacle_'];
