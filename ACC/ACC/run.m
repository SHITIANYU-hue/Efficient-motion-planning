function [flag_output, output_args] = run(ego, obs, lane_width )
    output_args = [];
    flag_output = true;
    close all;
    %% initial condition
...
%% while loop
    ...
    %        A(i*nx+1:(i+1)*nx,(N+1)*nx+i*nu+1:(N+1)*nx+(i+1)*nu) = BB1;
        ...
        for i = 1:1:N
            ...
            if is_exist_3 == true
                F = F + implies(-y((i-1)*nx+2) - 1.7/2 <= -lane_width/2, y((i-1)*nx+1) >= 3 * (obs(8)/3.6 + obs(9) * (i - 1)*dt) + x2_obstacle(i));
            end
    ...
    %         F = F + implies(-y((i-1)*nx+2) <= -1.5, y((i-1)*nx+1) + 1 <= x4_obstacle(i) | y((i-1)*nx+1) >= 1 + x4_obstacle(i));
        ...
        if is_exist_3 == true
            F = F + implies(-y((i-1)*nx+2) - 1.7/2 <= -lane_width/2, x2_obstacle(i) + 3 * (obs(8)/3.6 + obs(9) * (i - 1)*dt) <= y((i-1)*nx+1));
        end
        F = F + [y(1) == x_0];
        F = F + [y(2) == y_0];
        F = F + [y(3) == ego/3.6];
